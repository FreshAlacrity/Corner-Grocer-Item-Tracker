...
void MenuSearchOption::Select() {
	...
	// Key checking method via https://stackoverflow.com/a/11765524
	if ((*m_inventoryMap).count(searchInput) > 0) {
		...
		// Note: Using Levenshtein distance to find the best match would expand the search functionality
	}
}